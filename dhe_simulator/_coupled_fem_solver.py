...
def _make_submesh(global_nodes, global_elements, elem_subset_idx):
    sub_elems_global = global_elements[elem_subset_idx]

    pts = global_nodes[sub_elems_global]
    v1 = pts[:, 1, :] - pts[:, 0, :]
    v2 = pts[:, 2, :] - pts[:, 0, :]
    v3 = pts[:, 3, :] - pts[:, 0, :]
    ...
    sub_nodes = global_nodes[node_local_to_global]
...
class CoupledCylinderFEMSolver:
    ...
    def __init__(self, coupled_mesh, geometry):
        ...
        global_nodes = coupled_mesh.nodes
        global_elements = coupled_mesh.elements
    ...
    def _get_boundary_facets(self, basis, name):
        ...
        face_nodes = mesh.facets[:, bfacets]
        ...
        all_nodes_flat = face_nodes.ravel()
        pts = mesh.p[:, all_nodes_flat].T
        in_boundary = pred_loose(pts[:, 0], pts[:, 1], pts[:, 2]).reshape(3, -1)
        mask = in_boundary.all(axis=0)
        return bfacets[mask]
    ...
    def _cross_block(self, interface_name, basis_test, basis_trial, delta):
        ...
        face_nodes_local = mesh_t.facets[:, facets]
    ...
    def assemble_system(self, rho_func, c_func, k_func,
                        alpha2, alpha3, U_z,
                        delta_0, delta_1, delta_2, delta_f, T_f):
    ...
    def _assemble_omega1(self, rho_func, c_func, k_func,
        ...
        b = self.basis1
        rho_c_interp = self._interp_field(
            b, lambda x, y, z: rho_func(x, y, z) * c_func(x, y, z))
    ...
    def solve(self, T0_func, dt, tf, t_save, Ts_func,
              T_int=285.0, t_on=0.0, t_off=None, stabilize_first=True):
